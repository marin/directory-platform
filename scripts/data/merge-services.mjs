...
function loadExtracted(slug) {
  const path = join(SCRAPES_DIR, slug, "services-extracted.json");
...
for (const slug of slugs) {
  ...
  if (!existsSync(entryPath)) {
    missing += 1;
    report.push({ slug, status: "missing_entry", services: extracted.services.length });
    continue;
  }

  const current = entrySchema.parse(JSON.parse(readFileSync(entryPath, "utf-8")));
  const offers = mergeServiceOffers(current.offers ?? [], extracted.services, current.name);
  if (offers.length === (current.offers ?? []).length) {
    skipped += 1;
    report.push({ slug, status: "skipped_no_new_services", services: extracted.services.length });
    continue;
  }
  ...
  report.push({
    ...
    servicesAdded: offers.length - (current.offers ?? []).length,
    offersTotal: offers.length,
    servicesUrl: extracted.servicesUrl,
  });
}

const reportPath = join(ROOT, "work/scrapes/services-merge-report.json");
if (!dryRun) {
  writeFileSync(
    reportPath,
    JSON.stringify(
...
console.log(
  `${dryRun ? "Would update" : "Updated"} ${updated} entries` +
    (skipped ? `, skipped ${skipped} (no new services after dedupe)` : "") +
